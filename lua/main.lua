...
local prevButton = ui.Button.Mesh(ui.Bounds(-0.3, 0.05, 0.06,   0.1, 0.15, 0.01))
prevButton.label:setText("Prev")
prevButton:setDefaultTexture(Jukebox.assets.prev)
local pauseButton = ui.Button.Mesh(ui.Bounds(0.0, 0.05, 0.06,   0.1, 0.15, 0.01))
pauseButton.label:setText("Play")
pauseButton:setDefaultTexture(Jukebox.assets.pause)
local nextButton = ui.Button.Mesh(ui.Bounds( 0.3, 0.05, 0.06,   0.1, 0.15, 0.01))
...
local volumeSlider = ui.Slider(ui.Bounds(0, -0.12, 0.04,   0.8, 0.08, 0.05))
...
app:scheduleAction(0.02, true, function()
    ...
    if leftTrack and left and right then
        app.client:sendAudio(leftTrack, left)
